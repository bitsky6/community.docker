...
- name: Build an image with secrets
  community.docker.docker_image_build:
    ...
    secret:
      - id: mysecret
        src: pass_form_vault or path/to/file
        type: password or file
'''
    ...
    def add_args(self, args):
        ...
        if self.secret:
            for secret in self.secret:
                secret_str = secret['id']
                if 'src' in secret:
                    secret_str += f",src={secret['src']}"
...
def handle_secrets(self):
        ...
        for secret in self.secret:
            if secret['type'] == 'file':
                ...
                continue
            elif secret['type'] == 'password':
                if 'value' not in secret:
                    self.fail("Secret of type 'password' must include a 'value'.")
                ...
                self.fail("Invalid secret type. Expected 'file' or 'password'.")
        return temp_files
